...
### Git Operations
- **Commit and Push**: `./commands/git-push.sh "Your commit message"` - Handles add, commit, and push in one command
- Script automatically appends "Generated by John @ YourDesign.co.za" to commit messages
...
- **Theme CSS Location**: Additional custom styles go in `/opt/lampp/htdocs/wecoza/wp-content/themes/wecoza_3_child_theme/includes/css/ydcoza-styles.css`
...
5. Check `includes/error-logs.txt` for plugin-specific error logs
